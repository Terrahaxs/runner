...
def step_impl(context):
    ...
    context.worker_callback = m.post(
        f"{settings.api_url}/runner/callback", json={})
...
def step_impl(context):
    assert isinstance(
        context.result, InvalidSignatureError), type(
        context.result)
...
def step_impl(context):
    assert isinstance(
        context.result, RepoNotAllowedError), type(
        context.result)
...
def step_impl(context):
    assert isinstance(
        context.result, ProjectNotAllowedError), type(
        context.result)
